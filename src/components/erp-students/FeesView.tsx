  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto p-4 md:p-8 animate-in fade-in duration-500 font-jost space-y-4">
      ...
      <div className="bg-white rounded-[16px] border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.04)] p-4 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-gray-900 tracking-tight uppercase">Finance & Fees</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-gray-100 p-6 rounded-[16px] flex flex-col justify-between shadow-[0_2px_10px_rgba(0,0,0,0.04)] relative overflow-hidden group">
          ...
          <div>
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Total Structured Fee</span>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-4xl font-black text-[#144835]">₹{grandTotalFees.toLocaleString()}</span>
            </div>
          </div>
        ...
        <div className="bg-white border border-gray-100 p-6 rounded-[16px] flex flex-col justify-between shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
          <div>
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Total Paid Amount</span>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-4xl font-black text-emerald-700">₹{paidAmount.toLocaleString()}</span>
            </div>
          </div>
          <div className="text-[10px] text-emerald-600 font-bold uppercase tracking-wider mt-4 flex items-center gap-1.5">
        ...
        <div className="bg-red-50/20 border border-dashed border-red-200 p-6 rounded-[16px] flex flex-col justify-between shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
          <div>
            <span className="text-[10px] font-bold text-red-800 uppercase tracking-wider">Net Due Balance</span>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-4xl font-black text-red-700">₹{pendingAmount.toLocaleString()}</span>
            </div>
          </div>
          <div className="text-[10px] text-red-600 font-bold uppercase tracking-wider mt-4 flex items-center gap-1.5">
      ...
      <div className="bg-white border border-gray-100 rounded-[16px] overflow-hidden shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
        <div className="px-6 py-4 border-b border-gray-100 bg-[#F8FAFB]">
          <h3 className="text-xs font-black text-[#144835] uppercase tracking-wider">Transaction Ledger</h3>
        </div>
            ...
            <tbody className="divide-y divide-gray-50 font-bold text-gray-700">
              {transactions.map((txn, idx) => (
                <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4 text-[#144835] font-black">{txn.id}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    {txn.status === "Success" ? (
                      <button 
                        ...
                        className="text-[#144835] hover:underline font-black uppercase tracking-wider text-[10px] flex items-center justify-end gap-1 ml-auto"
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white border border-gray-100 rounded-[16px] w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="text-sm font-black text-[#144835] uppercase tracking-wider">Simulated Payment Portal</h3>
                <p className="text-[10px] text-gray-400 font-bold uppercase mt-1">Outstanding Balance: ₹{pendingAmount.toLocaleString()}</p>
              </div>
              <button 
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[9px] font-black text-gray-400 uppercase tracking-wider block mb-2">Payment Amount (INR)</label>
                <input 
                  type="text" 
                  disabled 
                  value={`₹${pendingAmount.toLocaleString()}`}
                  className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-sm font-black text-[#144835]"
                />
              </div>

              <div>
                <label className="text-[9px] font-black text-gray-400 uppercase tracking-wider block mb-2">Select Payment Method</label>
                <div className="grid grid-cols-3 gap-3">
                  {["UPI", "Net Banking", "Card"].map(m => (
                    <button
                      key={m}
                      onClick={() => setPaymentMethod(m)}
                      className={`py-3 rounded-lg border text-[11px] font-black uppercase tracking-wider transition-all ${
                        paymentMethod === m 
              ...
              <div className="bg-emerald-50/50 p-4 border border-emerald-100 rounded-lg mt-4">
                <p className="text-[10px] text-emerald-800 font-bold leading-normal uppercase">
                  🔒 Encrypted Payment System
                </p>
                <p className="text-[9px] text-emerald-600 font-semibold mt-0.5 leading-normal">
              ...
              <button
                ...
                className="w-full py-3.5 bg-[#144835] hover:bg-[#144835]/90 text-white rounded-lg text-xs font-bold uppercase tracking-widest transition-colors shadow-md mt-4"
              >
                Authorize & Pay Now
              </button>
            </div>
