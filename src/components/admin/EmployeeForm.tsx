...
export default function EmployeeForm({
  mode,
  employeeId,
  directoryHref = "/idpskalaburagi/hr/employees",
  category,
}: {
  mode: "create" | "edit";
  employeeId?: string;
    ...
    <form
      ...
      onSubmit={async (e) => {
        ...
        try {
          ...
          if (mode === "create") {
            const res = await fetch("/api/idpskalaburagi/employees", {
              method: "POST",
            ...
            const res = await fetch(`/api/idpskalaburagi/employees?id=${encodeURIComponent(form.employeeId)}`, {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify(payload),
            });
          ...
          const targetBase = isTeachingFromFields(payload.roleTitle, payload.department)
            ? "/idpskalaburagi/hr/teaching-staff"
            : "/idpskalaburagi/hr/non-teaching-staff";
