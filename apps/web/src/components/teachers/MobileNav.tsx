...
export default function MobileNav({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const activeGroupId = useMemo(() => {
    const group = navGroups.find((g) =>
      g.items.some((item) => pathname === item.href || (item.href !== "/schools/idpskalaburagi/teachers" && pathname.startsWith(item.href)))
    );
          ...
          <div className="space-y-2">
            {navGroups.map((group) => {
              const groupActive = group.items.some(
                (item) => pathname === item.href || (item.href !== "/schools/idpskalaburagi/teachers" && pathname.startsWith(item.href))
              );
                  ...
                  {isOpen ? (
                    <div className="p-2 border-t border-slate-200 bg-slate-50/30 space-y-1">
                      {group.items.map((item) => {
                        const active =
                          pathname === item.href || (item.href !== "/schools/idpskalaburagi/teachers" && pathname.startsWith(item.href));
                        return (
                          <Link
                            key={item.href}
                            href={item.href}
