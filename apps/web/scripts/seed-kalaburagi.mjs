...
async function main() {
  ...
  await writeCollection(
    "invoices",
    financeInvoices.map((x) => ({
      id: x.id,
      doc: {
        ...x,
        createdAt: x.date || now,
  ...
  await writeCollection(
    "applications",
    admissionApplications.map((x) => ({
      id: x.id,
      doc: {
        studentName: x.name || x.studentName,
