...
export default function Sidebar({
  ...
  const activeGroupId = useMemo(() => {
    const group = navGroups.find((g) =>
      g.items.some((item) => pathname === item.href || (item.href !== "/idpskalaburagi/students" && pathname.startsWith(item.href)))
    );
          ...
          <div className="space-y-1">
            {navGroups.map((group) => {
              const groupActive = group.items.some(
                (item) => pathname === item.href || (item.href !== "/idpskalaburagi/students" && pathname.startsWith(item.href))
              );
                  ...
                  {sidebarExpanded && isOpen ? (
                    <div className="pl-4 space-y-1">
                      {group.items.map((item) => {
                        const isActive =
                          pathname === item.href || (item.href !== "/idpskalaburagi/students" && pathname.startsWith(item.href));
                        return (
                            <Link
                              key={item.href}
                              href={item.href}
