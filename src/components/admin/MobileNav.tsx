...
export default function MobileNav({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const activeGroupId = useMemo(() => {
    const group = navGroups.find((g) =>
      g.items.some((item) => pathname === item.href || (item.href !== "/idpskalaburagi/admin" && pathname.startsWith(item.href)))
    );
          ...
          <div className="space-y-2">
            {navGroups.map((group) => {
              const groupActive = group.items.some(
                (item) => pathname === item.href || (item.href !== "/idpskalaburagi/admin" && pathname.startsWith(item.href))
              );
