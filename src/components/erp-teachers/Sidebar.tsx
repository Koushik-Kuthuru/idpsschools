  ...
  return (
    ...
    >
      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-white/10 relative overflow-hidden shrink-0">
        ...
        <Link 
          href="/idpscherukupalli/teachers" 
          onClick={(e) => {
            if (!sidebarExpanded) {
              e.preventDefault();
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 space-y-1 scrollbar-thin scrollbar-thumb-white/20 hover:scrollbar-thumb-white/40">
        ...
        <div>
          ...
          <div className="space-y-1">
            {navigation.map((item) => {
              const isActive = pathname === item.href || (item.href !== "/idpscherukupalli/teachers" && pathname.startsWith(item.href));
              return (
                <Link
                  key={item.name}
                  href={item.href}
      ...
      <div className="p-4 border-t border-white/10 bg-black/20 shrink-0">
        <Link 
          href="/idpscherukupalli/teachers/settings" 
          className={cn(
