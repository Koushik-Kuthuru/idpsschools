            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={cn(
                    "flex items-center py-2.5 rounded-lg text-xs font-medium transition-all group relative",
                    sidebarExpanded ? "gap-3 px-4 justify-start" : "px-3 justify-center",
                    isActive
      ...
      <div className="p-3 border-t border-white/10 bg-black/20 shrink-0">
        ...
        >
          <div className="relative shrink-0">
            <div className="h-8 w-8 rounded-full bg-[#144835] border-2 border-white/20 shadow-lg group-hover:border-[#a2c144] transition-colors flex items-center justify-center text-white font-bold text-xs">
              SA
            </div>
            <div className="absolute bottom-0 right-0 h-2.5 w-2.5 bg-green-500 border-2 border-[#0f3628] rounded-full"></div>
          </div>
          <div className={cn(
            "flex flex-col transition-opacity duration-300",
            sidebarExpanded ? "opacity-100 delay-100" : "opacity-0 w-0 overflow-hidden lg:hidden"
