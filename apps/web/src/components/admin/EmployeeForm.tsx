...
function inferSchoolIdFromHref(href: string) {
  const v = String(href || "");
  const m = v.match(/\/schools\/([^/]+)\/admin\//);
  if (m?.[1]) return m[1];
  const m2 = v.match(/^\/(idps[^/]+)\//);
  if (m2?.[1]) return m2[1];
  return "idpskalaburagi";
}
...
export default function EmployeeForm({
  ...
  const directoryLabel =
    directoryHref.includes("teaching-staff")
      ? "Teaching Staff"
      : directoryHref.includes("non-teaching-staff")
        ? "Non-Teaching Staff"
        : "Directory";
  ...
  return (
        ...
        try {
          ...
          if (mode === "create") {
            ...
            if (form.createUserAccount) {
              const staffRole = inferRoleFromStaff(payload.roleTitle, payload.department, category);
              ...
              if (provision.ok) {
                const credMsg = `Login created (${staffRole})\nEmail: ${provision.loginEmail || provision.email}\nPassword: ${portalPassword}`;
          ...
          const teaching = isTeachingFromFields(payload.roleTitle, payload.department);
          const staffSegment = teaching ? "teaching-staff" : "non-teaching-staff";
          router.push(`/schools/${schoolId}/admin/hr/${staffSegment}/${encodeURIComponent(form.employeeId)}/profile`);
        } catch (e: any) {
    ...
    >
      ...
      {provisionInfo && (
        <div className="rounded-[16px] border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs font-bold text-emerald-800">
          {provisionInfo}
        </div>
      )}
