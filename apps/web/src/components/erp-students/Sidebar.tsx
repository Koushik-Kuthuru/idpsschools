...
import { getStudentNavigation, type StudentNavItem } from "./navigation";
...
export default function Sidebar({
  ...
  const navigation = useMemo(() => getStudentNavigation(schoolId), [schoolId]);
  const dashboardHref = `/schools/${schoolId}/students`;
  const sidebarExpanded = isSidebarOpen || isMobileMenuOpen;

  const userInitials = useMemo(() => {
    const name = getStudentDisplayName(user as Parameters<typeof getStudentDisplayName>[0]);
    return name
      .split(" ")
      .filter(Boolean)
      .map((w) => w[0])
  ...
  const roleLabel = role ? getRoleLabel(role) : "Student";
  const avatarSrc = (user as { photoURL?: string; photo?: string })?.photoURL || (user as { photo?: string })?.photo || null;
        ...
        >
          <div className="relative shrink-0">
            {avatarSrc ? (
              <img
                src={avatarSrc}
                alt={getStudentDisplayName(user as Parameters<typeof getStudentDisplayName>[0])}
