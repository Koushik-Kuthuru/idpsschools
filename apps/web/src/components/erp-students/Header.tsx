...
import { getStudentNavigation } from "./navigation";
...
export default function Header({ setIsMobileMenuOpen }: HeaderProps) {
  ...
  const headerTitle = useMemo(() => {
    if (pathname.includes("/students/profile")) return "My Profile";
    if (pathname.includes("/students/help")) return "Help Center";
    if (pathname.includes("/students/notifications")) return "Notifications";
    const nav = getStudentNavigation(schoolId);
    const exact = nav.find((n) => n.href === pathname);
    if (exact) return exact.name;
    const nested = nav.find((n) => n.href !== `/schools/${schoolId}/students` && pathname.startsWith(n.href));
    return nested?.name ?? "Dashboard";
  }, [pathname, schoolId]);

  const helpHref = `/schools/${schoolId}/students/help`;
  const settingsHref = `/schools/${schoolId}/students/profile`;
  const profileHref = `/schools/${schoolId}/students/profile`;
  ...
  const userDisplayName = useMemo(
    () => getStudentDisplayName(user as Parameters<typeof getStudentDisplayName>[0]),
    [user]
  );

  const userDesignation = role === "student" ? "Student" : role ? role.charAt(0).toUpperCase() + role.slice(1) : "Student";

  const avatarSrc =
    (user as { photoURL?: string; photo?: string })?.photoURL ||
  ...
  return (
    <>
      <header className="relative h-16 bg-white/95 backdrop-blur-sm border-b border-gray-200/50 sticky top-0 z-30 px-4 sm:px-6 transition-all duration-300">
        <div className="grid h-full w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-3 md:grid-cols-[auto_minmax(0,1fr)_auto] md:gap-3 lg:gap-4">
          <div className="flex items-center gap-3 min-w-0 max-w-[9rem] sm:max-w-[11rem] lg:max-w-[13rem] shrink-0">
            ...
            <h2 className="text-sm sm:text-lg font-bold text-[#1A1A1A] truncate">{headerTitle}</h2>
          </div>
          ...
          >
            ...
            <div className="border-t border-gray-100 px-4 py-3 bg-gray-50/40">
              <button
                ...
                onClick={() => {
                  setNotificationsOpen(false);
                  router.push(`/schools/${schoolId}/students/notifications`);
                }}
