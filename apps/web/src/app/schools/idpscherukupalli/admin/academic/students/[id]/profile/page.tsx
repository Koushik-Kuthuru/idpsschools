...
const handlePhotoRemove = async (type: 'student' | 'father' | 'mother' | 'guardian') => {
 ...
 // Real data logic
 const academicYearStart = 2026;
 const year = monthIdx < 9 ? academicYearStart : academicYearStart + 1;
 const monthNumber = (monthIdx + 3) % 12 + 1;
 const dateStrIso = `${year}-${monthNumber.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
 
 const dateObj = new Date(year, monthNumber - 1, day);
 const isSunday = dateObj.getDay() === 0;
 
 // Using student.attendance array/object
 const isAbsent = student.attendance?.absentDates?.includes(dateStrIso);
 const isPresent = student.attendance?.presentDates?.includes(dateStrIso);
 
 // Check for invalid dates in the month (e.g. Feb 30th)
 const daysInMonth = new Date(year, monthNumber, 0).getDate();
 const isInvalidDate = day > daysInMonth;
